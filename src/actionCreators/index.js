...
import { selectRandom, selectRoulette } from '../lib/utilities';
import createExpressionLibrary from '../lib/expressions';

export default (getRandomInteger, getRandomReal, addToWorkerQueue, push) => {
    ...
    const actions = {
         ...
         */
        generateIndividuals: (numIndividuals, minExpressionDepth, maxExpressionDepth) => {

            const expressions = createExpressionLibrary(getRandomInteger, getRandomReal);
            const getToken = expressions.getToken.bind(null, expressions.tokenCreators, selectRandom.bind(null, getRandomInteger));
            const buildExpression = expressions.buildExpression.bind(null, getToken, getRandomInteger);
            ...
            return {
                ...
                individuals: new Array(numIndividuals).fill(1).map(n => {
                    return {
                        generationId: 1,
                        id: id++,
                        expression: buildExpression(minExpressionDepth, maxExpressionDepth),
         ...
         */
        generateSamples: (generation, numSamples, width, height, redThreshold, greenThreshold, blueThreshold, lastSampleId) => {
        ...
        generateSampleData: (sample, coordinateType) => {
            return (dispatch) => {
